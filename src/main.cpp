...
double averageExperiment(int length, const std::vector<bool>& lights, int iterations) {
    long long totalOps = 0;

    for (int iter = 0; iter < iterations; iter++) {
        std::vector<bool> shuffledLights = lights;
        if (lights.size() > 1) {
            unsigned seed = std::chrono::system_clock::now().time_since_epoch().count() + iter;
            std::shuffle(shuffledLights.begin(), shuffledLights.end(), std::mt19937(seed));
        }
...
int main() {
    ...
    for (int length = 2; length <= 1000; length += 2) {
        ...
        csvFile << length << ";" << avgOff << ";" << avgOn << ";" << avgRandom << "\n";
    }
    csvFile.close();
    return 0;
}
